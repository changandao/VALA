...
# def extract_gaussian_features_omniseg3d(model_path, iteration, source_path, views, gaussians, pipeline, background, feature_level):
...
def process_scene_language_features_robust(dataset : ModelParams, opt : OptimizationParams, iteration : int, pipeline : PipelineParams, feature_level : int, weight_threshold : float = 1e-5):
    """
    Process scene language features using robust Weiszfeld geometric median aggregation
    ...
    with torch.no_grad():
        ...
        extract_gaussian_features_robust(args.model_path, iteration, dataset.source_path, scene.getTrainCameras(), gaussians, pipeline, background, feature_level, weight_threshold)
