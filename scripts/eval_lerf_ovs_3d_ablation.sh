...
do
    ...
    python -m eval_ablation.render_lerf_by_text_ablation -s "$ROOT_DIR/dataset/3dgs/lerf_ovs/$case" -m "$ROOT_DIR/output/3dgs/$OUTPUT_DIR/$case" \
    --iteration 30000 --mask_thresh $mask_thresh --scene_name $case --dataset_name lerf_ovs --ablation_type ablation_stageA_plus_B \
...
python -m eval_ablation.compute_lerf_iou_ablation  --gt_dir "$ROOT_DIR/output/3dgs/lerf_ovs" \
--pred_dir "$ROOT_DIR/output/3dgs/$OUTPUT_DIR" --output_dir "$ROOT_DIR/output/3dgs/$ablation_output_dir" --iteration 30006 --mask_thresh $mask_thresh \
--ablation_type ablation_stageA_plus_B   