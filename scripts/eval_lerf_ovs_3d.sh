#!/bin/bash
# Evaluate open-vocabulary segmentation on LERF_OVS: render per-text relevancy maps,
# then compute IoU / localization metrics against the labeled frames.
...
do
    ...
    --base_dir "$BASE_DIR" --output_dir "$BASE_DIR" --eval --skip_train
done
