...
def evaluate(feat_dir, output_path, ae_ckpt_path, json_folder, mask_thresh, encoder_hidden_dims, decoder_hidden_dims, logger, direct_512=False):
    ...
    # Load features: either compressed (3D) or direct 512D
    last_dim = 3 if not direct_512 else 512
    sem_feats_all = np.zeros((len(feat_dir), len(eval_index_list), *image_shape, last_dim), dtype=np.float32)
    for i in range(len(feat_dir)):
        ...
        for j, idx in enumerate(eval_index_list):
            sem_feats_all[i][j] = np.load(feat_paths_lvl[idx])

    # instantiate autoencoder and openclip
    clip_model = OpenCLIPNetwork(device)
    if not direct_512:
        from autoencoder.model import Autoencoder  # lazy import to allow --direct_512 without this dependency
    ...
    for j, idx in enumerate(tqdm(eval_index_list)):
        ...
        with torch.no_grad():
            lvl, h, w, c = sem_feat.shape
            if direct_512:
                restored_feat = sem_feat  # already 512D features
            else:
                restored_feat = model.decode(sem_feat.flatten(0, 2))
                restored_feat = restored_feat.view(lvl, h, w, -1)           # 3xHxWx512
...
if __name__ == "__main__":
    ...
    parser.add_argument('--direct_512', action='store_true', help='Directly read 512D features and skip autoencoder decoding')
    ...
    evaluate(feat_dir, output_path, ae_ckpt_path, json_folder, mask_thresh, args.encoder_dims, args.decoder_dims, logger, direct_512=args.direct_512)