...
def test(args):
    ...
    # elif os.path.exists(args.label_src):
    else:
        print(args.label_src)
        with open(args.label_src, 'r') as f:
            labels_json = json.load(f)
        # Process the loaded JSON data based on your expected format
        # You might need to extract the labels from the JSON and tokenize them
        # labelset = labels_json.get('labels', [])  # Adjust according to your JSON structure
        label_ids = labels_json.get('classes', [])
        if 0 in label_ids:
            del label_ids[0]
        elif '0' in label_ids:  # Check string version too
            del label_ids['0']
        labelset = [scannet19_dict[id] for id in label_ids]
        print(labelset)
